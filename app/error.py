...
class CommandErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        e = discord.Embed(title=f'{type(error).__name__}: {error}',
            description="".join(traceback.format_tb(error.__traceback__)), color=0xD32F2F)#.set_footer(text='Please report this error to staff.')

        await ctx.send('❗_Komala Error_', embed=e)

        #await ctx.send(f'❗Komala Error:\n{type(error.__name__)}: {error}\n`{"".join(traceback.format_tb(error.__traceback__))}`')
